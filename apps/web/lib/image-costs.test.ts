...
import { estimateImageGenerationCost, resolveStoredImageModel } from "./image-costs";
...
describe("integrated image model cost estimates", () => {
  ...
  it("prefers the selected imageModel when backfilling provenance", () => {
    expect(resolveStoredImageModel(IMAGE_MODEL_GPT_IMAGE_2, undefined)).toBe(IMAGE_MODEL_GPT_IMAGE_2);
    expect(resolveStoredImageModel(IMAGE_MODEL_GEMINI_PRO, "old-label")).toBe(IMAGE_MODEL_GEMINI_PRO);
    expect(resolveStoredImageModel(undefined, IMAGE_MODEL_GEMINI_FLASH)).toBe(IMAGE_MODEL_GEMINI_FLASH);
    expect(resolveStoredImageModel()).toBe("gemini-3.1-flash-image-preview");
  });
});
